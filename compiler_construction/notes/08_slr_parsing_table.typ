...
+ State $i$ is constructed from $I_i$. The parsing actions for state $i$ are determined as follows:
  ...
  If any conflicting actions are generated by the above rules, we say that the grammar is not SLR(1), in which case the algorithms fails to produce a parser.
