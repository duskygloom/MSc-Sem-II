...
2. For each leader its basic block consists of the leader and all the statements upto but not including the next leader or the end of the program.\
  Example: The following statements of source code. It computes the dot product of two vectors of length 20. A list of three-address statements performing this computation is also shown.\
  Source code:
  ```
  begin
    prod = 0
...
- A basic block constitutes a set of expressions.
...
+ Interchange statements
  - Suppose we have two adjacent statements t1 = b + c and t2 = x + y.
  - We can interchange the two statements without affecting the value of the block if and only if neither x nor y is t1 and neither b or c is t2.

=== Algebraic transformation
+ Elimination of statements like `x = x + 0` and `x = x * 1`
...
Consider the following three-address code and their DAG.
```
t1 = 4 * i
t2 = a[t1]
t3 = 4 * i
